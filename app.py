...
def plot_k_line(df, code, name, mark_date=None):
    """
    绘制交互式 K 线图
    优化点：平移拖拽、滚轮缩放、十字光标、去除底部滑块
    """
    # 默认显示最近半年的数据，避免数据量过大导致卡顿，但允许用户缩放查看更多
    if mark_date:
    ...
    fig = make_subplots(
        ...
        row_width=[0.2, 0.8] # K线占80%，成交量占20%
    )

    # 1. 绘制 K 线 (使用 dict 结构修复警告)
    fig.add_trace(go.Candlestick(
        ...
        close=df_plot['close'],
        name='K线',
        increasing=dict(line=dict(color='#e53935'), fillcolor='#e53935'), # 红涨
        decreasing=dict(line=dict(color='#43a047'), fillcolor='#43a047')  # 绿跌
    ), row=1, col=1)
    ...
    fig.update_layout(
        ...
        plot_bgcolor='rgba(250,250,250,1)', # 极淡的背景
        xaxis_rangeslider_visible=False,   # 隐藏底部那个烦人的滑块
        dragmode='pan',                    # 【关键】默认鼠标左键是“平移”而不是“框选”
        hovermode='x unified',             # 【关键】十字光标，统一显示x轴信息
    ...
    fig.update_xaxes(
        rangebreaks=[dict(values=dt_breaks)], # 去除周末
        showspikes=True, spikethickness=1, spikecolor="gray", spikemode="across", # 十字准星线
        rangeselector=dict(
            buttons=list([
    ...
    fig.update_yaxes(showspikes=True, spikethickness=1, spikecolor="gray", spikemode="across") # Y轴也有准星
...
with col_list:
    ...
    if selected_row is None:
        if not df_fav.empty: selected_row = df_fav.iloc[0]
        elif not df_oth.empty: selected_row = df_oth.iloc[0]
